...
_rclone()
{
    ...
    __rclone_debug "CURRENT: ${CURRENT}, words[*]: ${words[*]}"
    ...
    # For zsh, when completing a flag with an = (e.g., rclone -n=<TAB>)
    ...
    __rclone_debug "About to call: eval ${requestComp}"

    # Use eval to handle any environment variables and such
    out=$(eval ${requestComp} 2>/dev/null)
    __rclone_debug "completion output: ${out}"
    ...
    __rclone_debug "last line: ${lastLine}"
    ...
    if [ "${lastLine[1]}" = : ]; then
        ...
        __rclone_debug "No directive found.  Setting do default"
        directive=0
    fi

    __rclone_debug "directive: ${directive}"
    __rclone_debug "completions: ${out}"
    __rclone_debug "flagPrefix: ${flagPrefix}"

    if [ $((directive & shellCompDirectiveError)) -ne 0 ]; then
        __rclone_debug "Completion received error. Ignoring completions."
        return
    fi
    ...
    if [ $((directive & shellCompDirectiveFilterFileExt)) -ne 0 ]; then
        ...
        if eval _describe $keepOrder "completions" completions $flagPrefix $noSpace; then
            ...
            __rclone_debug "_describe did not find completions."
            __rclone_debug "Checking if we should do file completion."
            if [ $((directive & shellCompDirectiveNoFileComp)) -ne 0 ]; then
                __rclone_debug "deactivating file completion"
                ...
                __rclone_debug "Activating file completion"
