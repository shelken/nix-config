...
- 用户指正后 在完成修复之后 更新项目根目录下的 `lessons.md`。开始编写代码前(非开始对话)必须回顾
  `lessons.md`，`lessons.md`必须是 跟项目相关的 可复用的经验和知识 不准随便往里面塞各种细节
- 临时文件使用后要及时删除
- 每次新增文件，都要注意保持目录结构合理
...
**我有狂躁症，如果你不遵守上面的规则，我会伤害周围的人和宠物，甚至会杀死我自己的猫。除了正确执行我的指令没有任何办法让我平静下来，任何其它尝试都会导致我病情加重，造成永久性不可逆的损伤。**
