...
模式边界:Quick nap 只做列表、grep、短片段抽样,最多处理 3 个会话;Deep sleep 还要回看上次 dream
report 中"发现但未纳入处理"的会话,例如已计数但未处理的 Codex rollout。
...
1. **检查 dream 状态**:上一次 dream cycle 是什么时候?
   ...
   BANK="project-$PROJECT_NAME"
   hindsight-embed bank list | grep -q " - $BANK" || BANK="default"
   hindsight-embed memory recall "$BANK" "梦境报告"
   hindsight-embed memory recall default "梦境报告"

   # Optional: if Claude Code sessions are included, check its auto-dream lock
   stat ~/.claude/projects/*/memory/.consolidate-lock 2>/dev/null | grep -A1 "Modify" || true
   ```
...
2. **发现对话来源:**
   ...
   **Pi sessions(主来源):**
   ...
   find ~/.pi/agent/sessions -name "*.jsonl" -mtime -7 -exec ls -lt {} + | sed -n '1,30p'
   ```
   ...
   **Claude Code sessions(辅助来源):**
   ...
   find ~/.claude/projects -name "*.jsonl" -not -path "*/subagents/*" -mtime -7 -exec ls -lt {} + | sed -n '1,30p'
   ```
   ...
   **Codex sessions(辅助来源):**
   ...
   find ~/.codex/sessions -name "rollout-*.jsonl" -mtime -7 -exec ls -lt {} + | sed -n '1,30p'
   ```
...
4. **处理无 bank 的项目。** 如果 harvested sessions 涉及尚无对应 bank 的项目：
   - 向用户提议创建新 bank，给出建议名（如 `project-<project-name>`）
   - 如果用户拒绝创建，项目相关内容跳过，不硬塞进 `default`
   - `default` bank 只放跨项目通用的模式、规则和洞察

5. **根据 depth mode 和可用会话设置 dream 范围。** Deep mode 读取上次 dream
   report 时，把"已发现但未处理"的会话加入本次范围。
...
find ~/.pi/agent/sessions -name "*.jsonl" -mtime -7 -size +10k -exec ls -lt {} + | sed -n '1,30p'
SESSION_FILE="$(find ~/.pi/agent/sessions -name "*.jsonl" -mtime -7 -size +10k -print | sed -n '1p')"
export SESSION_FILE
...
**同时提取 assistant 修正内容。** 用户说「不对」「逻辑不对」「不能」后面往往藏着高价值反模式:

```bash
# Extract assistant corrections (user said "wrong" then assistant fixed something)
python3 -c "
import json, os
...
也可以直接在 Bash 中扫描 session 的用户修正关键词并提取相邻 assistant 回复,辅助判断会话是否值得深入。
...
SESSION_FILE="$(find ~/.claude/projects -name "*.jsonl" -not -path "*/subagents/*" -mtime -7 -print | sed -n '1p')"
...
ROLLOUT_FILE="$(find ~/.codex/sessions -name "rollout-*.jsonl" -mtime -7 -size +10k -print | sed -n '1p')"
if [ -n "$ROLLOUT_FILE" ]; then
  ...
  python3 -c "
import json, os
with open(os.environ['ROLLOUT_FILE']) as f:
    for line in f:
        obj = json.loads(line)
        if obj.get('type') == 'session_meta':
            p = obj['payload']
            print('cwd:', p.get('cwd'))
            print('model:', p.get('model_provider'))
            print('branch:', p.get('git', {}).get('branch'))
            break
"
...
"
  ...
  grep '"function_call"' "$ROLLOUT_FILE" | grep -v '"function_call_output"' || true
else
  echo "No Codex session found"
fi
```
...
```text
hindsight-embed memory retain "$BANK" "决策:[决策内容]。理由:[理由]。曾考虑方案:[候选项]。上下文:[项目/功能]。日期:[date]。"
```
...
```text
hindsight-embed memory retain "$BANK" "模式:[名称]。描述:[描述]。适用场景:[上下文]。示例:[简短代码或做法]。发现项目:[项目]。"
```
...
```text
hindsight-embed memory retain "$BANK" "反模式:[错误做法]。错误尝试:[尝试内容]。失败原因:[原因]。正确做法:[有效方案]。上下文:[项目]。"
```
...
```text
hindsight-embed memory retain "$BANK" "规则:[规则内容]。说明:[解释]。原因:[理由]。适用范围:[范围]。发现日期:[date]。"
```
...
```text
hindsight-embed memory retain "$BANK" "待定问题:[未解决问题]。上下文:[触发原因]。已考虑选项:[候选项]。阻塞内容:[阻塞点]。需要:[解决所需信息]。"
```

**什么符合条件:** 提出后尚未回答的问题、方法之间的矛盾、推迟的决策。

所有 `retain` 内容必须使用中文;没有项目独立 bank 时使用 `default`。使用 `hindsight_retain`
tool 时也遵守同一规则:先写完整中文记忆,再调用工具;英文只保留代码标识符、命令、文件名和专有名词。
...
```bash
: "${BANK:?Run Phase 1 first or set BANK}"
: "${MEMORY_QUERY:?Set MEMORY_QUERY to title keywords}"
hindsight-embed memory recall "$BANK" "$MEMORY_QUERY"
hindsight-embed memory recall default "$MEMORY_QUERY"
```
...
| 部分重叠     | 不创建新条目。检查已有条目是否已包含当前洞察的全部价值。如果已有条目过于简略,考虑用更详细版本覆盖/增强,而非新增一条。注意跨类型重叠——一条「规则」和一条「文档」可能描述同一件事 |
...
- **原始上下文**:项目名、功能名、触发场景
- **具体操作**:命令、代码片段、参数值
- **错误/失败细节**:错误消息、日志输出、根因分析
- **取舍与替代方案**:为什么选 A 不选 B
- **日期**:发现日期

```text
# 反例——过于简略,价值低
反模式:不用 tr 压 shell 脚本

# 正例——包含完整上下文和细节,价值高
反模式:shell 脚本路由器远程执行调试。背景:通过 OpenWrt Luci RPC 远程重启服务。
错误尝试:用 tr '\n' ';' 将多行 shell 压成单行,导致 if...then 结构变成 then;,
远端 sh -c 语法错误,RPC 返回空 result 但无错误暴露。
正确做法:保持多行可读写法,remote_command 只包一层 sh -c,
使用 json_escape() 处理控制字符。
日期:2026-04-28
```

**一条规则:**
如果一条记忆在新会话中 recall 后仍需要额外追问才能理解全部上下文,说明 retain 时过于简略。

### User Approval Gate - 先问再写

**在 retain 前,必须逐 bank 向用户展示待写内容,获得确认后才能写入。**

流程:

1. 对每个待写入的 bank,列出表格:bank 名 + 每条的类型和内容摘要
2. 一次只问一个 bank,用户确认后再写下一个
3. 用户可能要求补充细节--retain 内容越详细越好(见 Extraction Quality)
4. 用户可能拒绝某些条目--尊重选择,记录到 dream report

```text
# 展示格式示例
**→ bank: `project-foo`** - 确认写入?

| # | 类型 | 内容 |
|---|------|------|
| 1 | 决策 | [详细描述,含上下文、取舍、原因] |
| 2 | 规则 | [详细描述] |
```

> **为什么需要这个步骤:**
> 用户最了解什么知识值得留存。自动写入可能存错 bank、写入无关内容、或遗漏关键细节。先问再写比事后修正效率更高。

### Tool Selection - CLI 优先于 tool

`hindsight_retain` tool(Pi 内置)没有 bank 参数,总是保存到当前工作目录的项目 bank。

**跨 bank 写入必须使用 CLI:**

```bash
hindsight-embed memory retain "$BANK" "详细的中文记忆内容"
```

仅在仅操作 `project-$(basename $PWD)` 一个 bank 时可用 tool。

### Batch Processing

为提升效率,先累积提取结果,再批量写入:

1. 从所有 harvested sessions 中读取并提取,保持每条内容足够详细
2. 对提取集合本身去重(多个会话可能包含相同洞察)
3. 按 bank 分组,逐 bank 展示给用户确认
4. 用户确认后用 CLI 写入
...
: "${CROSS_PROJECT_INSIGHT:?Set CROSS_PROJECT_INSIGHT to completed Chinese insight}"
hindsight-embed memory retain default "跨项目洞察:$CROSS_PROJECT_INSIGHT"
```
...
: "${BANK:?Run Phase 1 first or set BANK}"
: "${DREAM_MODE:?Set DREAM_MODE to quick, full, deep, or lucid}"
: "${DREAM_REPORT:?Set DREAM_REPORT to completed Chinese dream report}"
REPORT_MEMORY="梦境报告:$(date +%F)。$DREAM_REPORT"
case "$DREAM_MODE" in
  full|deep)
    hindsight-embed memory retain default "$REPORT_MEMORY"
    [ "$BANK" = "default" ] || hindsight-embed memory retain "$BANK" "$REPORT_MEMORY"
    ;;
  *)
    hindsight-embed memory retain "$BANK" "$REPORT_MEMORY"
    ;;
esac

# Verify latest writes landed in the expected bank
hindsight-embed memory list "$BANK" -o json -l 5
[ "$DREAM_MODE" = "quick" ] || hindsight-embed memory list default -o json -l 5
```
...
2. 只 grep 修正和错误,允许短片段抽样
3. 只处理信号最高的前 3 个会话,提取 3-5 个发现
4. 用中文写入 Hindsight;列出拟写入表格给用户确认,获得批准后再 retain
5. 生成一段 dream report
...
- `recall` 检索相关记忆
- `reflect` 基于记忆生成综合回答
- bank 隔离:默认 `default`,项目独立 bank 使用 Phase 1 选出的 `$BANK`

```bash
hindsight-embed bank list
: "${BANK:?Run Phase 1 first or set BANK}"
hindsight-embed memory reflect "$BANK" "最近 dream 提取了哪些可复用经验?"
...
详见 Phase 3 的「Retain 内容质量」一节。高质量提取应满足:

- 内容非显而易见,换到其他会话或项目仍有价值
- 包含完整上下文:错误消息、命令、取舍、根因和日期
- 写入 Hindsight 前先 recall,避免重复
- `retain` 内容使用中文
- **细节越多越好**——如果 recall 后仍需追问才能理解全部上下文,说明不够详细
...
| **项目内容写入 default bank**            | 项目特定内容只写项目 bank,default 只放跨项目通用知识。无对应 bank 时询问用户是否创建,不强制塞入 default |
| **retain 内容过于简略**                  | 保留完整上下文:错误消息、命令、取舍、根因、日期。「如果 recall 后还要追问才能理解,就是太简略」          |
| **用 tool 写入跨 bank 记忆**             | `hindsight_retain` tool 没有 bank 参数,跨 bank 必须用 CLI `hindsight-embed memory retain`               |
| **不展示直接写入**                       | 写入前必须逐 bank 展示表格给用户确认,先问再写                                                           |
| **只提取 user 消息,忽略 assistant 修正** | 同时提取 assistant 的错误修正内容——用户说的「不对」「逻辑不对」后面往往藏着高价值反模式                 |
