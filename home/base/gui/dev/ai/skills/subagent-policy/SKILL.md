...
4. 派发失败（exit 1）当回合处理：reason 前缀 `config-error:` 跳过该模型；`transient:` 可重试派发一次，二次失败跳过。失败 pane 已由脚本自动关闭
...
- 复审时可使用 intercom 与子代理进行交流; **主代理有任何审查方面的不同意见, 可直接与子代理辩论相关问题, 直达到成共识**
