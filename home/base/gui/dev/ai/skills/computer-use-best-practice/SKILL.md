...
- 启动会抢焦点的 GUI 应用前，先用 `focus-guard <seconds> &` 武装焦点保护，再启动目标应用；guard 在指定秒数内拦截所有抢焦点行为并切回原前台
...
2. `focus-guard <seconds> &` — 后台启动，记录当前前台并订阅焦点通知
3. 启动目标应用（`open -a` / spawn / 其它）
4. 目标应用抢焦点 → guard 收到 `didActivateApplicationNotification` → 立即 `activate` 切回原前台
